...
public:
  ...
  static void printHeader(RJunglePar &par, RJungleIO &io, time_t &start) {
		io.outVerbose->width(16);
    *(io.outVerbose) << "Start: " << ctime(&start)
        << "+---------------------+-----------------+-------------------+"
        << std::endl 
				<< "|    RandomJungle     |      " << par.version
        << "    |        2011       |" << std::endl
        << "+---------------------+-----------------+-------------------+"
        << std::endl
        ...
        << std::endl
    //<< "+-----------------------------------------------------------|" << std::endl
        //<< "|  For documentation, citation & bug-report instructions:  |" << std::endl
        << "+-----------------------------------------------------------+"
        << std::endl << std::endl << "Output to: " << par.outprefix << ".*"
#ifdef HAVE_MPI
        << std::endl << "MPI enabled."
#endif
  ...
  static void printFooter(
      RJunglePar &par, RJungleIO &io, time_t &start, time_t &end,
			clock_t &startgrow, clock_t &endgrow) {
    *io.outVerbose << std::endl << "Growing time: " << (double)(endgrow - startgrow)/CLOCKS_PER_SEC  << " sec" << std::endl;
  ...
  static void printRJunglePar(RJunglePar &par, std::ostream &os) {
    os << "file: " << par.filename << std::endl << "delimiter: "
        ...
        << "plugin: " << par.plugin << std::endl << "colselection: "
        ...
        << "extractdata: " << par.extractdata_flag << std::endl << "yaimp: "
        << par.yaimp_flag << std::endl << "seeed: " << par.seed << std::endl
        ...
        << par.targetPartitionSize << std::endl << "targetpluginPar: "
        << par.pluginPar << std::endl << std::endl;
  }

  //TODO: refresh xml entries
  static void printXmlHeader(RJunglePar &par, std::ostream &os) {
    os << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << std::endl
        ...
        << std::endl << "<option id=\"plugin\">" << par.plugin << "</option>"
        ...
        << std::endl << "<option id=\"pluginpar\">" << par.pluginPar
        << "</option>" << std::endl << "<option id=\"maxtreedepth\">"
