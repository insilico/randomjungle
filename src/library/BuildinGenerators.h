...
public:
  ...
  TermClassAtom<T, uli_t> *(*getTermResult)(DataFrame<T> &data, RJunglePar &par, uli_t col,
	...
	namespace Generator {
		...
		namespace CART {
			template<class T>
				...
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTgini<T>;
		...
		namespace CARTsse {
			template<class T>
				 ...
				 */
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTginisse<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
					...
					genFct->classifyIDcmpldTree = CmplFct::classifyT2nodeID<T>;
					genFct->newImportanceObject = TImportance<T>::newImportanceObject;
					genFct->checkUsability = FittingFct::checkUsabilitySse<T>;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = CmplFct::addVarProxT2<T>;
					genFct->setMtry = RJungleHelper<T>::setMtryClassification;
					genFct->getCutoffs = CmplFct::getCutoffsT2<T>;
		...
		namespace CARTtwoing {
			template<class T>
				 ...
				 */
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTtwoing<T>;
					genFct->getNodePerformance = Helper::purityTwoing<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
		...
		namespace CARTreg {
			template<class T>
				 ...
				 */
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTreg<T>;
		...
		namespace CARTregTwoing {
			template<class T>
				 ...
				 */
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTregTwoing<T>;
					genFct->getNodePerformance = Helper::puritySos<T>;
					genFct->getTermResult = TermResult::getTermMean<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpldSos;
					genFct->cmplTree = CmplFct::S<T>;
		...
		namespace CARTsrt {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTginiSrtOld<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
		...
		/** 
		 * \brief Experimental fitting function
		 * 
		 * @param genFct Generator object to be set.
		 */
		namespace CARTcor {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTginiCor<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
		...
		/** 
		 * \brief Conditional Inference trees for SNP data
		 * 
		 * @param genFct Generator object to be set.
		 */
		namespace CISNP {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = CondInf<T>::CISNP;
					genFct->getNodePerformance = Helper::one<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
					genFct->classifyCmpldTree = CmplFct::classifyT2<T>;
					genFct->classifyIDcmpldTree = CmplFct::classifyT2nodeID<T>;
					genFct->newImportanceObject = TImportance<T>::newImportanceObject;
					genFct->checkUsability = CondInf<T>::checkForTwoSampleData;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = NULL;
					genFct->setMtry = RJungleHelper<T>::setMtryClassification;
					genFct->getCutoffs = CmplFct::getCutoffsT2<T>;
		...
		/** 
		 * \brief Conditional Inference trees. Restrictions as follows:
		 *
		 * Variables:
		...
		namespace CICase1 {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = CondInf<T>::CICase1;
					genFct->getNodePerformance = Helper::one<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
					...
					genFct->classifyIDcmpldTree = CmplFct::classifyT2nodeID<T>;
					genFct->newImportanceObject = TImportance<T>::newImportanceObject;
					genFct->checkUsability = CondInf<T>::checkForCase1;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = NULL;
					genFct->setMtry = RJungleHelper<T>::setMtryClassification;
					genFct->getCutoffs = CmplFct::getCutoffsT2<T>;
		...
		namespace CICase3 {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = CondInf<T>::CICase3;
					genFct->getNodePerformance = Helper::puritySos<T>;
					genFct->getTermResult = TermResult::getTermMean<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpldSos;
					genFct->cmplTree = CmplFct::T2<T>;
					...
					genFct->classifyIDcmpldTree = CmplFct::classifyT2nodeID<T>;
					genFct->newImportanceObject = TImportance<T>::newImportanceObject;
					genFct->checkUsability = CondInf<T>::checkForCase3;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = NULL;
					genFct->setMtry = RJungleHelper<T>::setMtryClassification;
					genFct->getCutoffs = CmplFct::getCutoffsT2<T>;
		...
		namespace UCART {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::unbiasedCARTgini<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
		...
		namespace SCART {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::SCARTgini<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = NULL;
					genFct->classifyCmpldTree = NULL;
					genFct->classifyIDcmpldTree = NULL;
					genFct->newImportanceObject = TImportance<T>::newImportanceObject;
					genFct->checkUsability = NULL;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = NULL;
		...
		namespace trendCART {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::trendCARTgini<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
		...
		namespace imputeTree {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::imputeTree<T>;
					genFct->getNodePerformance = Helper::purityGini<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::T2<T>;
		...
		namespace CARTtwoWayIntAdd {
			template<class T>
				void assignFunctions(BuildinGenFct<T> *genFct) {
					genFct->getBestVariable = FittingFct::CARTtwoWayIntAdd<T>;
					genFct->getNodePerformance = Helper::purityTwoing<T>;
					genFct->getTermResult = TermResult::getTermMostFreq<T>;
					genFct->getAccuracyOfCmpldTrees = RJungleAcc<T>::getAccuracyCmpld;
					genFct->cmplTree = CmplFct::IAM2Way<T>;
					genFct->classifyCmpldTree = CmplFct::classifyIAM2Way<T>;
					genFct->classifyIDcmpldTree = CmplFct::classifyIAM2WaynodeID<T>;
					genFct->newImportanceObject = IAM2WayImportance<T>::newImportanceObject;
					genFct->checkUsability = NULL;
					genFct->getCenter = Helper::getMedian<T>;
					genFct->addVarProx = NULL;
					genFct->setMtry = RJungleHelper<T>::setMtryClassification;
