...
class Helper {
	public:
		...
		template<class T>
			static double performLrAndGetDeviance(
		...
		template<class T>
			static double purityGini(
				...
				while (it != rowMaskVec.end()) {
					val = data.at(*it, col);
#ifdef _HELPERDEBUG
					std::cout << "data[" << *it << "," << col << "] = " << val << std::endl;
#endif
					if (val != data.getMissingCode()) {
						classVec[val] += rowWeight[*it];
						n += rowWeight[*it];
#ifdef _HELPERDEBUG
						std::cout << "classVec[" << val << "] update: " << classVec[val] << std::endl;
						std::cout << "rowWeight[" << *it << "]: " << rowWeight[*it] << std::endl;
#endif
					}
#ifdef _HELPERDEBUG
					std::cout << "n update: " << n << std::endl;
#endif
					++it;
				}
				...
				for (iter = classVec.begin(); iter != classVec.end(); ++iter) {
					...
					//dummy = (double)iter->second;
					gini -= dummy * dummy;
#ifdef _HELPERDEBUG
					std::cout << "gini update: " << gini << std::endl;
#endif
				}
		 ...
		 */
		template<class T>
			static double purityTwoing(
					// There does not exists a consistent definition of
					// purity in a twoing tree (one should use gini instead)
		...
		//  template<class T>
		//  static T getMean(std::vector<T> &dataVec) {
		//    double mean = 0;
		//    typename std::vector<T>::iterator it = dataVec.begin();
		//
		//    while (it != dataVec.end()) {
			...
			static void getPowerSet(boost::dynamic_bitset<> &powerSet,
				...
				if (i != powerSet.npos)
					do {
						...
						std::cout << "powerSet.find_first() = " << powerSet.find_first() << std::endl;
#endif
						to.push_back(from[i]);
		...
		template<class T>
			static void getSNPsInLD(DataFrame<T> &data, unsigned int col, std::vector<
				...
				// buch inke koenig
				// r^2 = ( p11p22-p12p21 ) / ( pA(1-pA)pB(1-pB) )
				// (???Am J Hum Genet. 2004 Jan;74(1):106-20. Epub 2003 Dec 15. ??? tagsnps)
/*
				unsigned int j, i;
				double A, B, N, n11, rsq, rsqn, rsqd;
				unsigned int nrow, ncol, curCol;
				unsigned int idxCell;
				bool allCols = (colMaskVec == NULL);
				T valCell;

				ldVec.clear();

				nrow = (data.isUnsupervised) ? (data.par.nrow / 2) : (data.par.nrow);
				N = nrow * 2;

				// get pA
				A = 0;
				valCell = 1;
				idxCell = data.find(valCell, data.varClassIndexer[col]);
				if (idxCell < data.varClassIndexer[col].size()) {
					if (!data.isUnsupervised)
						A += data.varCategories[col][idxCell];
					else
						A += data.varHalfCategories[col][idxCell];
				}
				...
				if (idxCell < data.varClassIndexer[col].size()) {
					if (!data.isUnsupervised)
						A += data.varCategories[col][idxCell] * 2;
					else
						A += data.varHalfCategories[col][idxCell] * 2;
				}

				if (allCols) {
					ncol = data.par.ncol;
				} else {
					ncol = colMaskVec->size();
				}
				...
				for (j = 0; j < ncol; j++) {
					if (allCols) {
						...
						curCol = colMaskVec->at(j);
					}
					if ((curCol == data.par.depVar) || (curCol == col))
						continue;
					...
					for (i = 0; i < nrow; ++i) {
						if (data.at(i, col) == 1) {
							if (data.at(i, curCol) == 1)
								++n11;
							if (data.at(i, curCol) == 2)
								++n11;
						} else if (data.at(i, col) == 2) {
							if (data.at(i, curCol) == 1)
								++n11;
							if (data.at(i, curCol) == 2)
								n11 += 2;
					...
					B = 0;
					valCell = 1;
					idxCell = data.find(valCell, data.varClassIndexer[curCol]);
					if (idxCell < data.varClassIndexer[curCol].size()) {
						if (!data.isUnsupervised)
							B += data.varCategories[curCol][idxCell];
						else
							B += data.varHalfCategories[curCol][idxCell];
					}
					...
					if (idxCell < data.varClassIndexer[curCol].size()) {
						if (!data.isUnsupervised)
							B += data.varCategories[curCol][idxCell] * 2;
						else
							B += data.varHalfCategories[curCol][idxCell] * 2;
					}

					// get nominator / denominator
					rsqn = pow(n11 * N - A * B, 2);
					rsqd = A * (N - A) * B * (N - B);

					// calculate r^2
					rsq = rsqn / rsqd;

					std::cout << "col1 " << col << std::endl;
					std::cout << "col2 " << curCol << std::endl;
					std::cout << "A " << A << std::endl;
					std::cout << "B " << B << std::endl;
					std::cout << "N " << N << std::endl;
					std::cout << "n11 " << n11 << std::endl;
					std::cout << "rsqn " << rsqn << std::endl;
					std::cout << "rsqd " << rsqd << std::endl;
					std::cout << "rsq " << rsq << std::endl;
					std::cout << "cutoffHighLD " << data.par.cutoffHighLD << std::endl;
					std::cout << std::endl;

					// if in high LD then save it
					//if (rsq > data.par.cutoffHighLD) ldVec.flip(curCol);
					ldVec[curCol] = rsq;
				}
*/
			}
		...
		static bool isGuessingLomNumeric(RJunglePar &par) {
			return (par.treeType == tt_CART) || (par.treeType == tt_CARTreg)
				|| (par.treeType == tt_CARTsrt) || (par.treeType == tt_CARTcor)
				|| (par.treeType == tt_CICase1) || (par.treeType == tt_CISNP);
		}
...
#ifdef _HELPERDEBUG
					std::cout << data.at(idx[order[i]], j1) << " (" << idx[order[i]] << ")"
						<< ":";
#endif
					if (cutoffs.size() > j) {
#ifdef _HELPERDEBUG
		...
		template<class T>
			static void putPairSecondToVec(
					std::vector<std::pair<T, uli_t> > &rowPairVec,
