...
enum TreeType {
  // CART
	...
	tt_CARTtwoing = 2,	  // Class. trees: y nominal, x nominal
	tt_CARTreg = 3,       // Reg.   trees: y numeric, x numeric
  tt_CARTregTwoing = 4, // Reg.   trees: y numeric, x nominal

  // CART extension
	tt_CARTsrt = 5,       // Class. trees: y nominal, x numeric (faster for many different values in x)
  tt_CARTcor = 6,       // Class. trees: y nominal, x numeric with stratum
	tt_CARTsse = 7,	      // Class. trees: y nominal, x numeric (sse)

  // Conditional Inference
  tt_CICase1 = 101,     // y (nominal), x (numeric)
  tt_CICase3 = 103,     // y (nominal), x (ordinal)
  tt_CISNP = 111,       // y (nominal && two-sample), x (numeric)

  // Experimental
  tt_twoWayIntTree = 201, // two Way Interaction trees
  tt_LOTUS = 205, // LOTUS (Tree presentation with CART)
  tt_UCART = 206, // Unbiased variable selection with CART with GINI-Index and no regression trees (categorical output)
  tt_SCART = 207,
  tt_trendCART = 208,
  tt_CARTregWithMiss = 209,
  tt_imputeTree = 210
};
