...
public:
  ...
  void printXml(std::ostream &output) {
    ...
    variableToXML<std::vector<std::vector<std::vector<T > > > >(values, "values", output);
    variableToXML<std::vector<std::vector<uli_t > > >(branches, "branches", output);
	 ...
	 */
 	std::ostream &print(std::ostream &os, bool isXML = false) const {
      ...
      os << "\t|\t";
 	    ...
 	    // print branches
 	    if (branches[i].size() != 0) os << branches[i][0];
	 ...
	 */
  void pushBackNode() {
    for (uli_t i = 0; i < varSize; ++i) varID[i].push_back(0);
    values.push_back(std::vector<std::vector<T > >(valueWidth, std::vector<T >(valueSize, 0)));
