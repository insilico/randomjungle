...
#ifdef RANDOMJUNGLEACCDEBUG
		*io.outVerbose << "Get classes." << std::endl;
#endif
    data->getCol(depVar, colVec);
    Helper::getClasses<T>(colVec, classVec);
    sort(classVec.begin(), classVec.end());

    //if (classVec.size() != 2) throw Exception(ERRORCODE_12);

#ifdef RANDOMJUNGLEACCDEBUG
		*io.outVerbose << "Build index table." << std::endl;
#endif
    // build index table
    jj = 0;
...
#ifdef RANDOMJUNGLEACCDEBUG
			*io.outVerbose << "sample " << j << std::endl;
#endif
...
#ifdef RANDOMJUNGLEACCDEBUG
			*io.outVerbose << "Get acc. of forest" << std::endl;
#endif
        // forest
        classRes.clear();
        for (m = 0; m < dataSet.getNtree(); ++m) {
#ifdef RANDOMJUNGLEACCDEBUG
					*io.outVerbose << "Get acc. of tree " << m << std::endl;
          *io.outVerbose << "dataSet.getNsmpl " << dataSet.getNsmpl() << std::endl;
          *io.outVerbose << "dataSet.getNtree " << dataSet.getNtree() << std::endl;
          *io.outVerbose << "dataSet[" << j << "," << m << "]" << std::endl;
#endif
          if (dataSet.at(j, m))
...
#ifdef RANDOMJUNGLEACCDEBUG
        *io.outVerbose << "majorityVote " << majorityVote << std::endl;
#endif
      }

#ifdef RANDOMJUNGLEACCDEBUG
      *io.outVerbose << "store results in table" << majorityVote << std::endl;
#endif
      // store results
...
#ifdef RANDOMJUNGLEACCDEBUG
          *io.outVerbose << "Not found in index vector" << std::endl;
#endif
...
#ifdef RANDOMJUNGLEACCDEBUG
        *io.outVerbose << "get index of reponse of sample j " << i << std::endl;
#endif
...
#ifdef RANDOMJUNGLEACCDEBUG
		*io.outVerbose << "Build result table." << std::endl;
#endif
